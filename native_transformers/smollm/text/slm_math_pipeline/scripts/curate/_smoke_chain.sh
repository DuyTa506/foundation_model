...
health() {  # report parquet file health under a dir
  ...
  find "$d" -name '*.parquet' -printf '%s\t%p\n' | while IFS=$'\t' read -r sz p; do
    status=$($PY - "$p" <<'EOF'
import sys
...
$PY scripts/curate/07_tokenize_pack.py \
  --input_dir "$ROOT/pii_clean" --output_dir "$ROOT/tokenized" \
